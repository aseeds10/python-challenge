...
with open(bankpath, encoding="utf-8") as bankfile:
    ...
    for row in bankreader:
        ...
        if previous_profit_loss is not None: #[]
            ...
            #add each change from each comparison to list with changes
...
#now that have list of the changes over the period, can just do min and max
#for greatest incr and decr.placing here so after loop ends
...
#using index function to get row/placement
...
# Add 1 to account for skipping the first row
date_greatest_increase = dates[index_greatest_increase+1]
date_greatest_decrease = dates[index_greatest_decrease+1]
